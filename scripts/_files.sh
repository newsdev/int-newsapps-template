...
function init_dirs {
    ...
    mkdir ../nyt-$NAME >>$LOGFILE_PATH 2>>$LOGFILE_PATH
}

function copy_files {
    echo "🚀    2. Copying and editing files"
    cp -R $TYPE/* ../nyt-$NAME >>$LOGFILE_PATH 2>>$LOGFILE_PATH
    mv ../nyt-$NAME/MYAPP ../nyt-$NAME/$NAME >>$LOGFILE_PATH 2>>$LOGFILE_PATH
    cp -R common/* ../nyt-$NAME/ >>$LOGFILE_PATH 2>>$LOGFILE_PATH
